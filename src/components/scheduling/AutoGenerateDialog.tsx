...
type Props = {
  ...
  onRun?: (payload: AutoGenPayload) => Promise<void> | void; // optional, notified AFTER success
  defaults: {
    school_year: string;
    semester: string;
  };
...
const AutoGenerateDialog: React.FC<Props> = ({ open, onOpenChange, onRun, defaults }) => {
  ...
  const canSubmit = days.length > 0 && !submitting;
  ...
  const handleRun = async () => {
    ...
    setSubmitting(true);
    try {
      console.log("[AutoGenerateDialog] Submitting payload:", payload);
      const res = await apiService.autoGenerateSchedules(payload);
      console.log("[AutoGenerateDialog] API result:", res);

      if (!res.success) {
        throw new Error(res.message || res.error || "Auto-generation failed");
      }
      ...
      // Let parent refresh tables, etc. AFTER success (optional)
      if (onRun) {
        await onRun(payload);
      }
      ...
      console.error("[AutoGenerateDialog] Failed:", err);
      toast({
        variant: "destructive",
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Select value={String(slotMinutes)} onValueChange={(v) => setSlotMinutes(parseInt(v))} disabled={submitting}>
                <SelectTrigger>
          ...
          <div className="flex items-center gap-2 rounded-md border p-3">
            <Checkbox
              id="onlyWithRooms"
              checked={onlyWithRooms}
              onCheckedChange={(v) => setOnlyWithRooms(Boolean(v))}
              disabled={submitting}
            />
          ...
          <div className="rounded-md bg-muted px-3 py-2 text-xs text-muted-foreground">
            Rules: classes only between 7:30 AM–4:30 PM, no same-time classes in the same section, no professor
            double-booking, no duplicate subject per section, each subject ≤ 4 hrs/week, and (when enabled) only sections
            that already have rooms.
          </div>
        </div>
